...
const TYPE_MUTATORS: [TypeMutator; 14] = [
    mutate_res,
    mutate_const,
    mutate_int,
    mutate_flags,
...
pub(crate) fn display_value_diff(old_val: &Value, new_val: &Value, target: &Target) -> String {
    ...
    match old_val.kind() {
        ...
        ValueKind::Char => {
            let old = old_val.checked_as_char();
            let new = new_val.checked_as_char();
            format!("'{}' -> '{}'", old, new)
        }
        ValueKind::Float => {
            let old = old_val.checked_as_float();
            let new = new_val.checked_as_float();
            format!("{} -> {}", old, new)
        }
        ValueKind::Ptr => {
