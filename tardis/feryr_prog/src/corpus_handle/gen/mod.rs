...
pub fn gen_call(ctx: &mut Context, rng: &mut RngType) {
    // select a system call id for system call generation
...
pub const GENERATOR: [Generator; 14] = [
    gen_res,
    gen_const,
    gen_int,
    gen_flags,
...
pub fn gen_ty_value(ctx: &mut Context, rng: &mut RngType, ty: &Type, dir: Dir) -> Value {
    ...
    if dir == Dir::Out && matches!(ty.kind(), Const | Int | Flags | Proc ) {
