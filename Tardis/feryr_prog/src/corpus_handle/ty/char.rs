...
macro_rules! char_format_attr_getter {
    () => {
        #[inline(always)]
        ...
        pub fn bitfield_off(&self) -> u64 {
            self.char_fmt.bitfield_off
        }

        #[inline(always)]
        pub fn bitfield_len(&self) -> u64 {
            self.char_fmt.bitfield_len
        }

        #[inline(always)]
        pub fn bitfield_unit(&self) -> u64 {
            // self.char_fmt.bitfield_unit
        ...
        pub fn bitfield_unit_off(&self) -> u64 {
            self.char_fmt.bitfield_unit_off
        }
...
macro_rules! default_char_value {
    () => {
        #[inline]
        pub fn default_value(&self, dir: Dir) -> Value {
            CharValue::new(self.id(), dir, self.default_special_value()).into()
        }

        #[inline(always)]
        pub fn is_default(&self, val: &Value) -> bool {
            val.checked_as_char().val == self.default_special_value()
        }
    };
}
...
impl CharType {
    ...
    char_format_attr_getter! {}

    default_char_value! {}

    #[inline(always)]
    pub fn char_fmt(&self) -> &CharFormat {
        &self.char_fmt
    }
...
eq_ord_hash_impl!(CharType);

#[derive(Debug, Clone)]
pub struct CharTypeBuilder {
    comm: CommonInfo,
    char_fmt: CharFormat,
    range: Option<RangeInclusive<u64>>,
    align: u64,
...
impl CharTypeBuilder {
    #[inline(always)]
    pub fn new(comm: CommonInfo) -> Self {
        Self {
            comm,
            char_fmt: CharFormat::default(),
            range: None,
            align: 0,
        }
    }
    ...
    #[inline(always)]
    pub fn char_fmt(&mut self, fmt: CharFormat) -> &mut Self {
        self.char_fmt = fmt;
        self
    }
    ...
    pub fn build(self) -> CharType {
        CharType {
            comm: self.comm,
            char_fmt: self.char_fmt,
            range: self.range,
            align: self.align,
        }
    }
