...
use crate::corpus_handle::{prog::Prog, syscall::SyscallId, target::Target, HashMap};
...
impl RelationWrapper {
    ...
    pub fn try_update<T>(&mut self, p: &Prog, idx: usize, pred: T) -> bool
    where
        T: FnMut(&Prog, usize) -> bool, // fn(new_prog: &Prog, index: usize) -> bool
    {
        // let mut inner = self.inner.write().unwrap();
        self.inner.try_update(p, idx, pred)
    }
