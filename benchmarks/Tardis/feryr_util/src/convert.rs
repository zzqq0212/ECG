...
pub fn description_json_to_ast(
    ...
    // new struct_info as OtherHashMap<String, Vec<(i32, String)>
    let struct_info = OtherHashMap::new();
    let syscalls = convert_syscall_json(syscalls_json, &tys, struct_info)?;
...
fn convert_syscall_json(
    ...
    struct_info: OtherHashMap<String, Vec<(i32, String)>>,
    ...
    for (id, syscall_js) in syscalls_json.iter().enumerate() {
        ...
        // load struct.json
        let syscall_name = get(syscall_js, "Name")?.as_str().unwrap();
        let a: Vec<(i32, String)> = Default::default();
        let call_pos = match struct_info.get(syscall_name) {
            Some(call_pos) => call_pos,
            None => &a,
        };
        for iter in call_pos {
            let (idx, struct_name) = (iter.0 as u64, iter.1.clone());
            builder.struct_pos.push((struct_name, idx));
        }
...
const KIND_CONVERTERS: [KindConvertor; 12] = [
    convert_const_ty,
    convert_int_ty,
    convert_flags_ty,
    convert_csum_ty,
...
fn convert_intfmt(val: &JsonValue) -> Result<IntFormat, LoadError> {
    ...
    // dbg!(val);
    // dbg!(fmt, bitfield_off, bitfield_len, bitfield_unit, bitfield_unit_off);
    Ok(IntFormat {
